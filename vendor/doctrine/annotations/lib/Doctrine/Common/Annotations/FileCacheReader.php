...
class FileCacheReader implements Reader
{
    ...
    public function __construct(
        ...
        if (
            !is_dir($cacheDir) &&
            !@mkdir($cacheDir, 0777 & ~$this->umask, true)
        ) {
            throw new InvalidArgumentException(
                sprintf(
    ...
    public function getPropertyAnnotations(ReflectionProperty $property)
    {
        ...
        $key =
            $this->classNameHashes[$class->name] . '$' . $property->getName();
...
EXCEPTION
        ...
        if ($written === false) {
            throw new RuntimeException(
                sprintf("Unable to write cached file to: %s", $tempfile)
            );
        }
        ...
        if (rename($tempfile, $path) === false) {
            ...
            throw new RuntimeException(
                sprintf("Unable to rename %s to %s", $tempfile, $path)
            );
        }
    }

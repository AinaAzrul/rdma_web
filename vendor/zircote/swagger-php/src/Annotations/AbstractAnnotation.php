...
abstract class AbstractAnnotation implements \JsonSerializable
{
            ...
            if (property_exists($this, $property)) {
                ...
                if (is_array($value)) {
                    foreach ($value as $key => $annotation) {
                        if (
                            is_object($annotation) &&
                            $annotation instanceof AbstractAnnotation
                        ) {
                            $this->$property[$key] = $this->nested(
                                $annotation,
                                $nestedContext
                            );
                        }
    ...
    public function mergeProperties($object): void
    {
        ...
        foreach ($object as $property => $value) {
            ...
            if ($currentValues[$property] !== $value) {
                ...
                $identity = method_exists($object, "identity")
                    ? $object->identity()
                    : get_class($object);
                $context1 = $this->_context;
                $context2 = property_exists($object, "_context")
                    ? $object->_context
                    : "unknown";
                if (
                    is_object($this->$property) &&
                    $this->{$property} instanceof AbstractAnnotation
                ) {
    ...
    public function toYaml($flags = null): string
    {
        ...
        return Yaml::dump(
            json_decode($this->toJson(JSON_INVALID_UTF8_IGNORE)),
            10,
            2,
            $flags
        );
    ...
    public function validate(
        ...
        foreach ($this->_unmerged as $annotation) {
            if (!is_object($annotation)) {
                $this->_context->logger->warning(
                    'Unexpected type: "' .
                        ...
                        '" in ' .
                        $this->identity() .
                        "->_unmerged, expecting a Annotation object"
                );
                break;
            }
            ...
            if ($details = static::matchNested($class)) {
                ...
                if (is_array($property)) {
                    $this->_context->logger->warning(
                        "Only one " .
                            Util::shorten(get_class($annotation)) .
                            "() allowed for " .
                            $this->identity() .
                            " multiple found, skipped: " .
                            $annotation->_context
                    );
                } else {
                    $this->_context->logger->warning(
                ...
                if ($class::$_parents) {
                    $message .=
                        ...
                        implode(", ", Util::shorten($class::$_parents));
                }
                $this->_context->logger->warning(
                    $message . " in " . $annotation->_context
                );
            }
            $valid = false;
        }
                ...
                if (is_array($item) && is_numeric($key) === false) {
                    $this->_context->logger->warning(
                        $this->identity() .
                            "->" .
                            $property .
                            " is an object literal, use nested " .
                            Util::shorten($annotationClass) .
                            "() annotation(s) in " .
                            $this->_context
                    );
                    $keys[$key] = $item;
            ...
            foreach (static::$_required as $property) {
                if (Generator::isDefault($this->$property)) {
                    ...
                    foreach (static::$_nested as $class => $nested) {
                        $nestedProperty = is_array($nested)
                            ? $nested[0]
                            : $nested;
        ...
        return self::_validate($this, $stack, $skip, $ref, $context)
            ? $valid
            : false;
    }
    ...
    private static function _validate(
        ...
        if (is_object($fields)) {
            ...
            $skip[] = $fields;
            $blacklist = property_exists($fields, "_blacklist")
                ? $fields::$_blacklist
                : [];
        }
        ...
        foreach ($fields as $field => $value) {
            ...
            $ref =
                $baseRef !== ""
                    ? $baseRef . "/" . urlencode((string) $field)
                    : urlencode((string) $field);
            if (is_object($value)) {
                if (method_exists($value, "validate")) {
                    ...
                    !self::_validate($value, $stack, $skip, $ref, $context)
                ) {
                    $valid = false;
                }
            } elseif (
        ...
        foreach (static::$_parents as $parent) {
            foreach ($parent::$_nested as $annotationClass => $entry) {
                if (
                    ...
                    is_array($entry) &&
                    !Generator::isDefault($this->{$entry[1]})
                ) {
                    $properties[] = $entry[1];
                    break 2;
                }
    ...
    public static function matchNested(string $class)
    {
        ...
        while (
            0 !== strpos($parent, "OpenApi\\Annotations\\") &&
    ...
    protected function _identity(array $properties): string
    {
        ...
        return Util::shorten(get_class($this)) .
            "(" .
            implode(",", $fields) .
            ")";
    }
    ...
    protected function nested(
        ...
        if (
            ...
            $annotation->_context === $this->_context
        ) {

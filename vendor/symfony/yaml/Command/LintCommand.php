...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (
            "github" === $this->format &&
            !class_exists(GithubActionReporter::class)
        ) {
            throw new \InvalidArgumentException(
                'The "github" format is only available since "symfony/console" >= 5.3.'
            );
        }
        ...
        if (null === $this->format) {
            ...
            $this->format =
                class_exists(GithubActionReporter::class) &&
                GithubActionReporter::isGithubActionEnvironment()
                    ? "github"
                    : "txt";
        }
    ...
    private function displayTxt(
        ...
        if (0 === $erroredFiles) {
            $io->success(
                sprintf("All %d YAML files contain valid syntax.", $countFiles)
            );
        } else {
            $io->warning(
                sprintf(
                    "%d YAML files have valid syntax and %d contain errors.%s",
                    $countFiles - $erroredFiles,
                    $erroredFiles,
                    $suggestTagOption
                        ? " Use the --parse-tags option if you want parse custom tags."
                        : ""
                )
            );
        }
    ...
    private function displayJson(SymfonyStyle $io, array $filesInfo): int
    {
        ...
        array_walk($filesInfo, function (&$v) use (&$errors) {
            ...
            if (
                isset($v["message"]) &&
                false !== strpos($v["message"], "PARSE_CUSTOM_TAGS")
            ) {
                $v["message"] .=
                    " Use the --parse-tags option if you want parse custom tags.";
            }

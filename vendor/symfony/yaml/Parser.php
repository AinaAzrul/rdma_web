...
class Parser
{
    ...
    public function parseFile(string $filename, int $flags = 0)
    {
        ...
        if (!is_readable($filename)) {
            throw new ParseException(
                sprintf('File "%s" cannot be read.', $filename)
            );
        }
    ...
    private function doParse(string $value, int $flags)
    {
        ...
        if (
            null !==
                ($tag = $this->getLineTag($this->currentLine, $flags, false)) &&
            !$this->moveToNextLine()
        ) {
        ...
        do {
            ...
            if (
                ...
                if (
                    ...
                    $data[] = $this->parseBlock(
                        $this->getRealCurrentLineNb() + 1,
                        $this->getNextEmbedBlock(null, true) ?? "",
                        $flags
                    );
                } elseif (
                    null !==
                    ($subTag = $this->getLineTag(
                        ltrim($values["value"], " "),
                        $flags
                    ))
                ) {
                ...
                if (!\is_string($key) && !\is_int($key)) {
                    throw new ParseException(
                        (is_numeric($key) ? "Numeric" : "Non-string") .
                    ...
                    if (
                        ...
                        if (!\array_key_exists($refName, $this->refs)) {
                            if (
                                ...
                                throw new ParseException(
                                    sprintf(
                                        'Circular reference [%s] detected for reference "%s".',
                                        implode(
                                            ", ",
                                            array_merge(
                                                \array_slice(
                        ...
                        if (
                            Yaml::PARSE_OBJECT_FOR_MAP & $flags &&
                            $refValue instanceof \stdClass
                        ) {
                        ...
                        if (isset($parsed[0])) {
                            ...
                            foreach ($parsed as $parsedItem) {
                                if (
                                    Yaml::PARSE_OBJECT_FOR_MAP & $flags &&
                                    $parsedItem instanceof \stdClass
                                ) {
                ...
                try {
                    $parsedMapping = Inline::parse(
                        $this->lexInlineMapping(),
                        $flags,
                        $this->refs
                    );
                ...
                try {
                    $parsedSequence = Inline::parse(
                        $this->lexInlineSequence(),
                        $flags,
                        $this->refs
                    );
                ...
                if (0 === $this->currentLineNb) {
                    ...
                    foreach ($this->lines as $line) {
                        ...
                        if ("" === $trimmedLine) {
                            ...
                            !$previousLineWasNewline &&
                            !$previousLineWasTerminatedWithBackslash
                        ) {
                            $value .= " ";
                        }
     ...
     */
    private function getNextEmbedBlock(
    ...
    private function parseValue(string $value, int $flags, string $context)
    {
        if (0 === strpos($value, "*")) {
            ...
            if (!\array_key_exists($value, $this->refs)) {
                if (
                    false !==
                    ($pos = array_search($value, $this->refsBeingParsed, true))
                ) {
                    throw new ParseException(
                        sprintf(
                            'Circular reference [%s] detected for reference "%s".',
                            implode(
                                ", ",
                                array_merge(
                                    \array_slice($this->refsBeingParsed, $pos),
                                    [$value]
                                )
                            ),
                            $value
        ...
        if (
            ...
            self::preg_match(
                "/^(?:" .
                    self::TAG_PATTERN .
                    " +)?" .
                    self::BLOCK_SCALAR_HEADER_PATTERN .
                    '$/',
                $value,
                $matches
            )
        ...
        try {
            if ("" !== $value && "{" === $value[0]) {
                $cursor =
                    \strlen(rtrim($this->currentLine)) - \strlen(rtrim($value));

                return Inline::parse(
                    $this->lexInlineMapping($cursor),
                    $flags,
                    $this->refs
                );
            ...
            switch ($value[0] ?? "") {
                ...
                case "'":
                    $cursor =
                        \strlen(rtrim($this->currentLine)) -
                        \strlen(rtrim($value));
                    $parsedValue = Inline::parse(
                        $this->lexInlineQuotedString($cursor),
                        $flags,
                        $this->refs
                    );

                    if (
                        isset($this->currentLine[$cursor]) &&
                        preg_replace(
                            '/\s*(#.*)?$/A',
                            "",
                            substr($this->currentLine, $cursor)
                        )
                    ) {
                        throw new ParseException(
                            sprintf(
     ...
     */
    private function parseBlockScalar(
        string $style,
        string $chomping = "",
        int $indentation = 0
    ): string {
        ...
        if (0 === $indentation) {
            ...
            for (
                $i = 0;
                $i < $currentLineLength && " " === $this->currentLine[$i];
                ++$i
            ) {
                ++$indentation;
            }
        ...
        if (">" === $style) {
            ...
            for (
                $i = 0, $blockLinesCount = \count($blockLines);
                $i < $blockLinesCount;
                ++$i
            ) {
                if ("" === $blockLines[$i]) {
    ...
    private function isNextLineIndented(): bool
    {
        ...
        do {
            ...
            !$EOF &&
            ($this->isCurrentLineEmpty() || $this->isCurrentLineComment())
        );
    ...
    private function lexInlineStructure(
        int &$cursor,
        string $closingTag
    ): string {
        $value = $this->currentLine[$cursor];
    ...
    private function consumeWhitespaces(int &$cursor): bool
    {
        ...
        do {
            $whitespaceOnlyTokenLength = strspn(
                $this->currentLine,
                " ",
                $cursor
            );

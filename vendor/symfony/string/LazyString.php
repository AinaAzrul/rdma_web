...
class LazyString implements \Stringable, \JsonSerializable
{
    ...
    public static function fromCallable($callback, ...$arguments): self
    {
        if (
            ...
            !(
                \is_array($callback) &&
                isset($callback[0]) &&
                $callback[0] instanceof \Closure &&
                2 >= \count($callback)
            )
        ) {
            throw new \TypeError(
                sprintf(
                    'Argument 1 passed to "%s()" must be a callable or a [Closure, method] lazy-callable, "%s" given.',
                    __METHOD__,
        ...
        $lazyString->value = static function () use (
            &$callback,
            &$arguments,
            &$value
        ): string {
    ...
    public static function fromStringable($value): self
    {
        if (!self::isStringable($value)) {
            throw new \TypeError(
                sprintf(
                    'Argument 1 passed to "%s()" must be a scalar or a stringable object, "%s" given.',
                    __METHOD__,
    ...
    public function __toString()
    {
        ...
        try {
            ...
            if (
                ...
                $e = new \TypeError(
                    sprintf(
                        "Return value of %s() passed to %s::fromCallable() must be of the type string, %s returned.",
                        $callback,
                        static::class,
                        $type
                    )

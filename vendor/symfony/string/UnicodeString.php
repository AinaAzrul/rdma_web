...
class UnicodeString extends AbstractUnicodeString
{
    ...
    {
        $this->string = normalizer_is_normalized($string)
            ? $string
    ...
    public function append(string ...$suffix): AbstractString
    {
        ...
        $str->string =
            $this->string .
            (1 >= \count($suffix) ? $suffix[0] ?? "" : implode("", $suffix));
        normalizer_is_normalized($str->string) ?:
        ($str->string = normalizer_normalize($str->string));
    ...
    public function endsWith($suffix): bool
    {
        ...
        $form =
            null === $this->ignoreCase ? \Normalizer::NFD : \Normalizer::NFC;
        normalizer_is_normalized($suffix, $form) ?:
        ($suffix = normalizer_normalize($suffix, $form));
            ...
            return 0 ===
                mb_stripos(
                    grapheme_extract(
                        ...
                        \strlen($this->string) - \strlen($suffix)
                    ),
                    $suffix,
                    0,
                    "UTF-8"
    ...
    public function equalsTo($string): bool
    {
        ...
        $form =
            null === $this->ignoreCase ? \Normalizer::NFD : \Normalizer::NFC;
        normalizer_is_normalized($string, $form) ?:
        ($string = normalizer_normalize($string, $form));

        if ("" !== $string && false !== $string && $this->ignoreCase) {
            return \strlen($string) === \strlen($this->string) &&
                0 === mb_stripos($this->string, $string, 0, "UTF-8");
        }
    ...
    public function indexOf($needle, int $offset = 0): ?int
    {
        ...
        $form =
            null === $this->ignoreCase ? \Normalizer::NFD : \Normalizer::NFC;
        normalizer_is_normalized($needle, $form) ?:
        ($needle = normalizer_normalize($needle, $form));
        ...
        try {
            $i = $this->ignoreCase
                ? grapheme_stripos($this->string, $needle, $offset)
                : grapheme_strpos($this->string, $needle, $offset);
        } catch (\ValueError $e) {
            return null;
        }
    ...
    public function indexOfLast($needle, int $offset = 0): ?int
    {
        ...
        $i = $this->ignoreCase
            ? grapheme_strripos($string, $needle, $offset)
            : grapheme_strrpos($string, $needle, $offset);
    ...
    public function normalize(int $form = self::NFC): parent
    {
        ...
        if (\in_array($form, [self::NFC, self::NFKC], true)) {
            normalizer_is_normalized($str->string, $form) ?:
    ...
    public function prepend(string ...$prefix): AbstractString
    {
        $str = clone $this;
        $str->string =
            (1 >= \count($prefix) ? $prefix[0] ?? "" : implode("", $prefix)) .
            $this->string;
        normalizer_is_normalized($str->string) ?:
        ($str->string = normalizer_normalize($str->string));
    ...
    {
        $str = clone $this;
        normalizer_is_normalized($from) ?:
        ($from = normalizer_normalize($from));

        if ("" !== $from && false !== $from) {
            $tail = $str->string;
            $result = "";
            $indexOf = $this->ignoreCase
                ? "grapheme_stripos"
                : "grapheme_strpos";
    ...
    public function slice(int $start = 0, int $length = null): AbstractString
    {
        ...
        if (
            ...
            0 > $start &&
            grapheme_strlen($this->string) < -$start
        ) {
            $start = 0;
        }
        $str->string = (string) grapheme_substr(
            $this->string,
            $start,
            $length ?? 2147483647
        );
    ...
    public function splice(
        ...
        $start = $start
            ? \strlen(grapheme_substr($this->string, 0, $start))
            : 0;
        $length = $length
            ? \strlen(
                grapheme_substr($this->string, $start, $length ?? 2147483647)
            )
            : $length;
        $str->string = substr_replace(
            $this->string,
            $replacement,
            $start,
            $length ?? 2147483647
    ...
    public function split(
        ...
        normalizer_is_normalized($delimiter) ?:
        ($delimiter = normalizer_normalize($delimiter));

        if (false === $delimiter) {
            throw new InvalidArgumentException(
                "Split delimiter is not a valid UTF-8 string."
            );
        }
    ...
    public function startsWith($prefix): bool
    {
        ...
        $form =
            null === $this->ignoreCase ? \Normalizer::NFD : \Normalizer::NFC;
        normalizer_is_normalized($prefix, $form) ?:
        ($prefix = normalizer_normalize($prefix, $form));
    ...
    public function __wakeup()
    {
        if (!\is_string($this->string)) {
            throw new \BadMethodCallException(
                "Cannot unserialize " . __CLASS__
            );
        }

        normalizer_is_normalized($this->string) ?:
        ($this->string = normalizer_normalize($this->string));
    }

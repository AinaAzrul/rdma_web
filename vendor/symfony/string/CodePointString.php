...
class CodePointString extends AbstractUnicodeString
{
    ...
    public function endsWith($suffix): bool
    {
        ...
        if ($this->ignoreCase) {
            return preg_match(
                "{" . preg_quote($suffix) . '$}iuD',
                $this->string
            );
        }

        return \strlen($this->string) >= \strlen($suffix) &&
            0 === substr_compare($this->string, $suffix, -\strlen($suffix));
    }
    ...
    public function indexOf($needle, int $offset = 0): ?int
    {
        ...
        $i = $this->ignoreCase
            ? mb_stripos($this->string, $needle, $offset, "UTF-8")
            : mb_strpos($this->string, $needle, $offset, "UTF-8");
    ...
    public function indexOfLast($needle, int $offset = 0): ?int
    {
        ...
        $i = $this->ignoreCase
            ? mb_strripos($this->string, $needle, $offset, "UTF-8")
            : mb_strrpos($this->string, $needle, $offset, "UTF-8");
    ...
    public function replace(string $from, string $to): AbstractString
    {
        ...
        if ($this->ignoreCase) {
            $str->string = implode(
                $to,
                preg_split("{" . preg_quote($from) . "}iuD", $this->string)
            );
        } else {
    ...
    public function splice(
        ...
        $str = clone $this;
        $start = $start
            ? \strlen(mb_substr($this->string, 0, $start, "UTF-8"))
            : 0;
        $length = $length
            ? \strlen(mb_substr($this->string, $start, $length, "UTF-8"))
            : $length;

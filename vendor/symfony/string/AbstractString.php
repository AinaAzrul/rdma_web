...
abstract class AbstractString implements \Stringable, \JsonSerializable
{
    ...
    public function bytesAt(int $offset): array
    {
        ...
        return "" === $str->string
            ? []
            : array_values(unpack("C*", $str->string));
    }
    ...
    public function endsWith($suffix): bool
    {
        if (!\is_array($suffix) && !$suffix instanceof \Traversable) {
            throw new \TypeError(
                sprintf(
                    'Method "%s()" must be overridden by class "%s" to deal with non-iterable values.',
                    __FUNCTION__,
                    static::class
                )
            );
    ...
    public function repeat(int $multiplier): self
    {
        if (0 > $multiplier) {
            throw new InvalidArgumentException(
                sprintf("Multiplier must be positive, %d given.", $multiplier)
            );
        }
    ...
    public function split(
        ...
        if (null === $flags) {
            throw new \TypeError(
                'Split behavior when $flags is null must be implemented by child classes.'
            );
        }
                ...
                foreach (get_defined_constants(true)["pcre"] as $k => $v) {
                    if ($lastError === $v && "_ERROR" === substr($k, -6)) {
                        throw new RuntimeException(
                            "Splitting failed with " . $k . "."
                        );
                    }
                }

                throw new RuntimeException(
                    "Splitting failed with unknown error code."
                );
            }
        } finally {
    ...
    public function toByteString(string $toEncoding = null): ByteString
    {
        ...
        $toEncoding = \in_array($toEncoding, ["utf8", "utf-8", "UTF8"], true)
            ? "UTF-8"
            : $toEncoding;
        ...
        if (
            ...
            $toEncoding ===
                ($fromEncoding =
                    $this instanceof AbstractUnicodeString ||
                    preg_match("//u", $b->string)
                        ? "UTF-8"
                        : "Windows-1252")
        ) {
     ...
     * @return static
     */
    abstract public function trimEnd(
        string $chars = " \t\n\r\0\x0B\x0C\u{A0}\u{FEFF}"
    ): self;
    ...
    public function trimPrefix($prefix): self
    {
        ...
        if (
            "" !== $prefix &&
            \strlen($this->string) >= \strlen($prefix) &&
            0 ===
                substr_compare(
                    $this->string,
                    $prefix,
     ...
     * @return static
     */
    abstract public function trimStart(
        string $chars = " \t\n\r\0\x0B\x0C\u{A0}\u{FEFF}"
    ): self;
    ...
    public function truncate(
        ...
        $ellipsisLength =
            "" !== $ellipsis ? (new static($ellipsis))->length() : 0;

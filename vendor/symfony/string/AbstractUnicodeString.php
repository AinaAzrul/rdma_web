...
abstract class AbstractUnicodeString extends AbstractString
{
        ...
        while (\strlen($s) - 1 > ($i = strspn($s, self::ASCII))) {
            ...
            if ($rule instanceof \Transliterator) {
                ...
                if ("nfd" === ($rule = strtolower($rule))) {
                    normalizer_is_normalized($s, self::NFD) ?:
                    ($s = normalizer_normalize($s, self::NFD));
                } elseif ("nfkd" === $rule) {
                    normalizer_is_normalized($s, self::NFKD) ?:
                    ...
                    static function ($c) {
                        ...
                        if (
                            ...
                            throw new \LogicException(
                                sprintf(
                                    '"%s" requires a translit-able iconv implementation, try installing "gnu-libiconv" if you\'re using Alpine Linux.',
                                    static::class
                                )
                            );
                        }
    ...
    public function codePointsAt(int $offset): array
    {
        ...
        foreach (
            preg_split("//u", $str->string, -1, \PREG_SPLIT_NO_EMPTY)
            as $c
        ) {
    ...
    public function folded(bool $compat = true): parent
    {
        ...
        if (
            ...
            !\defined("Normalizer::NFKC_CF")
        ) {
            $str->string = normalizer_normalize(
                $str->string,
                $compat ? \Normalizer::NFKC : \Normalizer::NFC
            );
            $str->string = mb_strtolower(
                str_replace(self::FOLD_FROM, self::FOLD_TO, $this->string),
                "UTF-8"
            );
        } else {
            $str->string = normalizer_normalize(
    ...
    public function join(array $strings, string $lastGlue = null): parent
    {
        ...
        $tail =
            null !== $lastGlue && 1 < \count($strings)
                ? $lastGlue . array_pop($strings)
                : "";
    ...
    public function match(
        ...
        $match =
            (\PREG_PATTERN_ORDER | \PREG_SET_ORDER) & $flags
                ? "preg_match_all"
                : "preg_match";
    ...
    public function replaceMatches(string $fromRegexp, $to): parent
    {
        ...
        if (\is_array($to) || $to instanceof \Closure) {
            if (!\is_callable($to)) {
                throw new \TypeError(
                    sprintf(
                        'Argument 2 passed to "%s::replaceMatches()" must be callable, array given.',
                        static::class
                    )
                );
            }
        ...
        try {
            if (
                null ===
                ($string = $replace($fromRegexp . "u", $to, $this->string))
            ) {
                $lastError = preg_last_error();
    ...
    private function pad(int $len, self $pad, int $type): parent
    {
        ...
        switch ($type) {
            case \STR_PAD_RIGHT:
                return $this->append(
                    str_repeat($pad->string, intdiv($freeLen, $padLen)) .
                        ($len ? $pad->slice(0, $len) : "")
                );
            ...
            case \STR_PAD_BOTH:
                ...
                $len = $rightLen % $padLen;
                $str = $this->append(
                    str_repeat($pad->string, intdiv($rightLen, $padLen)) .
                        ($len ? $pad->slice(0, $len) : "")
                );
                ...
                return $str->prepend(
                    str_repeat($pad->string, intdiv($leftLen, $padLen)) .
                        ($len ? $pad->slice(0, $len) : "")
                );
    ...
    private function wcswidth(string $string): int
    {
        ...
        foreach (preg_split("//u", $string, -1, \PREG_SPLIT_NO_EMPTY) as $c) {
            ...
            if (null === self::$tableZero) {
                self::$tableZero = require __DIR__ .
                    "/Resources/data/wcswidth_table_zero.php";
            }

            if (
                $codePoint >= self::$tableZero[0][0] &&
                $codePoint <=
                    self::$tableZero[
                        ($ubound = \count(self::$tableZero) - 1)
                    ][1]
            ) {
                $lbound = 0;
                while ($ubound >= $lbound) {
            ...
            if (null === self::$tableWide) {
                self::$tableWide = require __DIR__ .
                    "/Resources/data/wcswidth_table_wide.php";
            }

            if (
                $codePoint >= self::$tableWide[0][0] &&
                $codePoint <=
                    self::$tableWide[
                        ($ubound = \count(self::$tableWide) - 1)
                    ][1]
            ) {
                $lbound = 0;
                while ($ubound >= $lbound) {

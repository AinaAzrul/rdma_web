...
class BufferedTokenStream implements TokenStream
{
    ...
    public function get(int $index): Token
    {
        ...
        if ($index < 0 || $index >= $count) {
            throw new \OutOfBoundsException(
                \sprintf("Token index %d out of range 0..%d.", $index, $count)
            );
        }
     ...
     * @return array<Token>
     */
    public function getHiddenTokensToRight(
        int $tokenIndex,
        int $channel
    ): ?array {
        $this->lazyInit();
        ...
        $to =
            $nextOnChannel === -1 ? \count($this->tokens) - 1 : $nextOnChannel;
    ...
    public function getHiddenTokensToLeft(int $tokenIndex, int $channel): ?array
    {
        ...
        $prevOnChannel = $this->previousTokenOnChannel(
            $tokenIndex - 1,
            Lexer::DEFAULT_TOKEN_CHANNEL
        );
     ...
     * @return array<Token>|null
     */
    protected function filterForChannel(
        int $left,
        int $right,
        int $channel
    ): ?array {
    ...
    {
        return $this->getTextByInterval(
            new Interval(0, \count($this->tokens) - 1)
        );
    }

    public function getTextByTokens(
        ?Token $start = null,
        ?Token $stop = null
    ): string {
        $startIndex = $start === null ? 0 : $start->getTokenIndex();
        $stopIndex =
            $stop === null ? \count($this->tokens) - 1 : $stop->getTokenIndex();

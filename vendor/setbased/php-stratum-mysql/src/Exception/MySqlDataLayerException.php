 ...
 */
class MySqlDataLayerException extends \RuntimeException implements
    DataLayerException
{
    //--------------------------------------------------------------------------------------------------------------------
    /**
    ...
    {
        return array_merge(
            $this->splitIntoTwoColumns("MySQL Errno", (string) $this->errno),
     ...
     * @return array
     */
    protected function splitIntoTwoColumns(
        string $header,
        string $string
    ): array {
        $lines = [];

...
and   table_name   = %s',
     ...
     * @throws MySqlQueryErrorException
     */
    public function labelsFromTable(
        string $tableName,
        string $idColumnName,
...
and   t1.ROUTINE_NAME   = '%s'",
    ...
    public function setCharacterSet(string $characterSet, string $collate): void
    {
        $sql = sprintf(
            ...
            $this->dl->quoteString($characterSet),
            $this->dl->quoteString($collate)
        );
...
order by ORDINAL_POSITION',
     ...
     * @throws MySqlQueryErrorException
     */
    public function tablePrimaryKey(
        string $schemaName,
        string $tableName
    ): array {
        $sql = sprintf(
...
where Key_name = \'PRIMARY\'',
     ...
     * @throws MySqlQueryErrorException
     */
    public function tableUniqueIndexes(
        string $schemaName,
        string $tableName
    ): array {
        $sql = sprintf(

    ...
    protected function writeResultHandler(): void
    {
        ...
        foreach ($this->routine["bulk_insert_keys"] as $i => $key) {
            if ($key != "_") {
                ...
                $columns .=
                    "`" .
                    $this->routine["bulk_insert_columns"][$i]["column_name"] .
                    "`";

                if ($fields) {
                    $fields .= ",";
                }
                $fields .= DataTypeHelper::escapePhpExpression(
                    $this->routine["bulk_insert_columns"][$i],
                    '$row[\'' . $key . '\']'
                );
            }
        }
        ...
        $this->codeStore->append(
            '$sql = "INSERT INTO `' .
                $this->routine["bulk_insert_table_name"] .
                "`(" .
                $columns .
                ')".PHP_EOL;'
        );

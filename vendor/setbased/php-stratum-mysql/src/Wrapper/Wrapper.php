...
abstract class Wrapper
{
     ...
     * @return Wrapper
     */
    public static function createRoutineWrapper(
        array $routine,
        PhpCodeStore $codeStore,
        NameMangler $nameMangler
    ): Wrapper {
    ...
    public function isBlobParameter(?array $parameters): bool
    {
        ...
        if (!empty($parameters)) {
            foreach ($parameters as $parameter) {
                $hasBlob =
                    $hasBlob ||
                    DataTypeHelper::isBlobParameter($parameter["data_type"]);
            }
        }
    ...
    public function writeRoutineFunctionWithLob(): void
    {
        ...
        $methodName = $this->nameMangler->getMethodName(
        ...
        foreach ($this->routine["parameters"] as $parameter) {
            if (DataTypeHelper::getBindVariableType($parameter) == "b") {
                $mangledName = $this->nameMangler->getParameterName(
                    $parameter["parameter_name"]
                );

                $this->codeStore->append(
                    '$this->sendLongData($stmt, ' .
                        $blobArgumentIndex .
                        ', $' .
                        $mangledName .
                        ");"
    ...
    protected function getRoutineArgs(): string
    {
        ...
        foreach ($this->routine["parameters"] as $parameter) {
            ...
            $ret .= DataTypeHelper::escapePhpExpression(
                $parameter,
                '$' . $mangledName
            );
        }
    ...
    protected function getWrapperArgs(): string
    {
        ...
        foreach ($this->routine["parameters"] as $parameter) {
            ...
            $declaration = DataTypeHelper::phpTypeHintingToPhpTypeDeclaration(
                $dataType . "|null"
            );
            if ($declaration !== "") {
                $code .= $declaration . " ";
            ...
            $code .=
                '$' .
                $this->nameMangler->getParameterName(
                    $parameter["parameter_name"]
                );
        }
    ...
    private function generatePhpDocBlockParameters(): void
    {
        ...
        if (!empty($parameters)) {
            ...
            foreach ($parameters as $parameter) {
                $maxNameLength = max(
                    $maxNameLength,
                    mb_strlen($parameter["php_name"])
                );
                $maxTypeLength = max(
                    $maxTypeLength,
                    mb_strlen($parameter["php_type"])
                );
            }

...
class FunctionWrapper extends Wrapper
{
    ...
    {
        $type = DataTypeHelper::phpTypeHintingToPhpTypeDeclaration(
            $this->getDocBlockReturnType()
        );
    ...
    protected function writeResultHandler(): void
    {
        ...
        if ($this->routine["return"] == "bool") {
            $this->codeStore->append(
                sprintf(
                    "return !empty(\$this->executeSingleton0('select %s(%s)'));",
            ...
            $this->codeStore->append(
                sprintf(
                    "return \$this->executeSingleton0('select %s(%s)');",
                    $this->routine["routine_name"],
                    $this->getRoutineArgs()
                )
            );

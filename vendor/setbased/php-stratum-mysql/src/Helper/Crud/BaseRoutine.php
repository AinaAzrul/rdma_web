...
abstract class BaseRoutine
{
    ...
    protected function generateDocBlockAllColumnsWithKeyList(): void
    {
        ...
        $format = sprintf(
            " * @param p_%%-%ds @todo describe parameter",
            $padding
        );
        foreach ($this->tableColumns as $column) {
            $this->codeStore->append(
                sprintf($format, $column["column_name"]),
                false
            );
        }
    ...
    protected function generateKeyListInDocBlock(): void
    {
        ...
        if (!empty($keys)) {
            ...
            $this->codeStore->append(" * ", false);
            $this->codeStore->append(
                " * NOTE: Table does not have a key.",
                false
            );
        }
    }
    ...
    {
        $this->codeStore->append(
            sprintf("create procedure %s(", $this->routineName)
        );

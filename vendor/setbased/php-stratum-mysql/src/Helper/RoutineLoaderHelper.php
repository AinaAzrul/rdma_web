...
class RoutineLoaderHelper
{
        ...
        foreach ($description as $column) {
            ...
            switch ($parts1[1]) {
                case "tinyint":
                    ...
                    $tmp["numeric_precision"] = Cast::toManInt(
                        $parts2["precision"] ?? 4
                    );
                    $tmp["numeric_scale"] = 0;
                    break;
                ...
                case "decimal":
                    ...
                    $tmp["numeric_scale"] = Cast::toManInt(
                        $parts2["scale"] ?? 0
                    );
                    break;
    ...
    public function loadStoredRoutine(): array
    {
        ...
        if ($load) {
            $this->io->text(
                sprintf(
                    "Loading routine <dbo>%s</dbo>",
                    OutputFormatter::escape($this->routineName)
                )
            );
    ...
    private function dropRoutineIfExists(): void
    {
        if (!empty($this->rdbmsOldRoutineMetadata)) {
            $this->dl->dropRoutine(
                $this->rdbmsOldRoutineMetadata["routine_type"],
                $this->routineName
            );
        }
    }
    ...
    private function extractBulkInsertTableColumnsInfo(): void
    {
        ...
        $tableIsNonTemporary = $this->dl->checkTableExists(
            $this->bulkInsertTableName
        );
        ...
        $this->bulkInsertColumns = self::extractColumnsFromTableDescription(
            $description
        );
    }
    ...
    private function extractDesignationType(): void
    {
        ...
        if (count($tags) === 0) {
            throw new RoutineLoaderException(
                "Tag @type not found in DocBlock."
            );
        } elseif (count($tags) > 1) {
            throw new RoutineLoaderException(
                "Multiple @type tags found in DocBlock."
            );
        }
        ...
        switch ($this->designationType) {
            case "bulk_insert":
                ...
                if (
                    $tag["arguments"]["table"] === "" ||
                    $tag["arguments"]["columns"] === "" ||
                    $tag["description"][0] != ""
                ) {
                    throw new RoutineLoaderException(
                        "Invalid @type tag. Expected: @type bulk_insert <table_name> <columns>"
                    );
                }
                $this->bulkInsertTableName = $tag["arguments"]["table"];
                $this->bulkInsertKeys = explode(
                    ",",
                    $tag["arguments"]["columns"]
                );
                break;
            ...
            case "rows_with_index":
                ...
                if (
                    ...
                    throw new RoutineLoaderException(
                        "Invalid @type tag. Expected: @type %s <columns>",
                        $this->designationType
                    );
                }
                $this->indexColumns = explode(
                    ",",
                    $tag["arguments"]["columns"]
                );
                break;
    ...
    {
        return [
            "short_description" => $this->docBlockReflection->getShortDescription(),
    ...
    {
        $this->routineParameters = new RoutineParametersHelper(
            $this->dl,
            $this->io,
            ...
            foreach ($matches[0] as $placeholder) {
                if (isset($this->replacePairs[strtoupper($placeholder)])) {
                    $this->replace[$placeholder] =
                        $this->replacePairs[strtoupper($placeholder)];
                } else {
                    $unknown[] = $placeholder;
                }
            }
    ...
    private function extractReturnType(): void
    {
        ...
        switch ($this->designationType) {
            ...
            case "singleton1":
                if (count($tags) === 0) {
                    throw new RoutineLoaderException(
                        "Tag @return not found in DocBlock."
                    );
                }
                $tag = $tags[0];
                if ($tag["arguments"]["type"] === "") {
                    throw new RoutineLoaderException(
                        "Invalid return tag. Expected: @return <type>."
                    );
                }
            ...
            default:
                if (count($tags) !== 0) {
                    throw new RoutineLoaderException(
                        "Redundant @type tag found in DocBlock."
                    );
                }
        }
    }
    ...
    {
        $n = preg_match(
            "/create\\s+(procedure|function)\\s+([a-zA-Z0-9_]+)/i",
            $this->routineSourceCode,
            $matches
        );
        if ($n == 1) {
            if ($this->routineName != $matches[2]) {
                throw new RoutineLoaderException(
                    "Stored routine name '%s' does not corresponds with filename",
                    $matches[2]
                );
            }
        } else {
            throw new RoutineLoaderException(
                "Unable to find the stored routine name and type"
            );
        }
    }
    ...
    private function logUnknownPlaceholders(array $unknown): void
    {
        ...
        $code = strtr(
            OutputFormatter::escape($this->routineSourceCode),
            $replace
        );
    ...
    private function mustLoadStoredRoutine(): bool
    {
        ...
        foreach (
            $this->phpStratumOldMetadata["replace"]
            as $placeHolder => $oldValue
        ) {
            if (
                !isset($this->replacePairs[strtoupper($placeHolder)]) ||
        ...
        if (
            !$this->sqlModeHelper->compare(
                $this->rdbmsOldRoutineMetadata["sql_mode"]
            )
        ) {
            return true;
        }
        ...
        if (
            $this->rdbmsOldRoutineMetadata["character_set_client"] !==
            $this->characterSet
        ) {
            return true;
        }
        ...
        if (
            $this->rdbmsOldRoutineMetadata["collation_connection"] !==
            $this->collate
        ) {
            return true;
        }
    ...
    private function readSourceCode(): void
    {
        ...
        DocBlockReflection::setTagParameters("paramAddendum", [
            "name",
            "type",
            "delimiter",
            "enclosure",
    ...
    private function substitutePlaceHolders(): string
    {
        ...
        $this->replace["__FILE__"] =
            "'" . $this->dl->realEscapeString($realpath) . "'";
        $this->replace["__ROUTINE__"] = "'" . $this->routineName . "'";
        $this->replace["__DIR__"] =
            "'" . $this->dl->realEscapeString(dirname($realpath)) . "'";
    ...
    private function updateMetadata(): void
    {
        ...
        $this->phpStratumMetadata[
            "parameters"
        ] = $this->routineParameters->getParameters();
        ...
        if ($this->designationType === "bulk_insert") {
            $this->phpStratumMetadata["bulk_insert_table_name"] =
                $this->bulkInsertTableName;
            $this->phpStratumMetadata["bulk_insert_columns"] =
                $this->bulkInsertColumns;
            $this->phpStratumMetadata["bulk_insert_keys"] =
                $this->bulkInsertKeys;
        }
    }
    ...
    private function validateReturnType(): void
    {
        ...
        if (
            ...
            throw new RoutineLoaderException(
                "Return type must be 'mixed', 'bool', or a combination of 'int', 'float', 'string', and 'null'"
            );
        }

        // The following tests are applicable for singleton0 routines only.
        if (!in_array($this->designationType, ["singleton0"])) {
            return;
        }
        ...
        if ($key === false) {
            throw new RoutineLoaderException(
                "Return type must be 'mixed', 'bool', or contain 'null' (with a combination of 'int', 'float', and 'string')"
            );
        }
    }

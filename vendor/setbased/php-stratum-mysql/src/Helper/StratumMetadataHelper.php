...
class StratumMetadataHelper
{
    ...
    {
        $this->routines = array_intersect_key(
            $this->routines,
            array_flip($routineNames)
        );
    }
    ...
    {
        $data = ["revision" => $this->revision, "routines" => $this->routines];

        $json = json_encode($data, JSON_PRETTY_PRINT);
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new RuntimeException(
                "Error of encoding to JSON: '%s'.",
                json_last_error_msg()
            );
        }
    ...
    private function readMetadata(): void
    {
        ...
        if (
            !is_array($data ?? null) ||
            ($data["revision"] ?? null) !== $this->revision
        ) {
            $data = null;
        }

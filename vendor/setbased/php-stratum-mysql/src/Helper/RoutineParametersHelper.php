...
class RoutineParametersHelper
{
    ...
    public function extractDocBlockPartsWrapper(): array
    {
        ...
        foreach ($this->parameters as $parameter) {
            $parameters[] = [
                ...
                "php_type" =>
                    DataTypeHelper::columnTypeToPhpTypeHinting($parameter) .
                    "|null",
        ...
        foreach ($this->parameters as $key => $parameter) {
            if ($parameter["parameter_name"]) {
                ...
                if (isset($parameter["character_set_name"])) {
                    $dataTypeDescriptor .=
                        " character set " . $parameter["character_set_name"];
                }
                if (isset($parameter["collation_name"])) {
                    $dataTypeDescriptor .=
                        " collation " . $parameter["collation_name"];
                }
        ...
        foreach ($this->parametersAddendum as $parameterName => $addendum) {
            ...
            foreach ($this->parameters as $key => $parameter) {
                if ($parameter["parameter_name"] === $parameterName) {
                    $this->parameters[$key] = array_merge(
                        $this->parameters[$key],
                        $addendum
                    );
                    $exists = true;
            ...
            if (!$exists) {
                throw new RoutineLoaderException(
                    "Specific parameter '%s' does not exist",
                    $parameterName
                );
            }
        }
    ...
    private function enhanceTypeOfParameters()
    {
        foreach ($this->parameters as $key => $parameter) {
            if ($parameter["data_type"] === "TYPE OF") {
                $n = preg_match(
                    '/^("(?<schema>[a-zA-Z0-9_]+)"\.)?("(?<table>[a-zA-Z0-9_]+)")\.("(?<column>[a-zA-Z0-9_]+)")%TYPE$/',
                    $parameter["dtd_identifier"],
                    $matches
                );
                if ($n !== 1) {
                    throw new RoutineLoaderException(
                        "Unable to parse data type description %s of parameter %s.",
                        $parameter["dtd_identifier"],
                ...
                $this->parameters[$key]["numeric_precision"] =
                    $column["numeric_precision"];
                $this->parameters[$key]["numeric_scale"] =
                    $column["numeric_scale"];
                $this->parameters[$key]["character_set_name"] =
                    $column["character_set_name"];
                $this->parameters[$key]["collation_name"] =
                    $column["collation_name"];
                $this->parameters[$key]["dtd_identifier"] =
                    $column["column_type"];
            }
        }
    }
    ...
    private function extractParametersAddendum(): void
    {
        ...
        foreach ($tags as $tag) {
            ...
            if (
                ...
                throw new RoutineLoaderException(
                    "Expected: @paramAddendum <field_name> <type_of_list> <delimiter> <enclosure> <escape>."
                );
            }

            if (isset($this->parametersAddendum[$parameterName])) {
                throw new RoutineLoaderException(
                    "Duplicate @paramAddendum tag for parameter '%s'",
                    $parameterName
                );
            }

            $this->parametersAddendum[$parameterName] = [
                "name" => $parameterName,
                "data_type" => $dataType,
                "delimiter" => $delimiter,
                "enclosure" => $enclosure,
    ...
    private function validateParameterLists(): void
    {
        ...
        foreach ($tmp as $name) {
            $this->io->logNote(
                "Parameter <dbo>%s</dbo> is missing from doc block",
                $name
            );
        }
        ...
        foreach ($tmp as $name) {
            $this->io->logNote(
                "Unknown parameter <dbo>%s</dbo> found in doc block",
                $name
            );
        }
    }

...
class ConvertTagsHelper
{
    ...
    private function InsertNewTags()
    {
        if (isset($this->tags["param"])) {
            $this->insertLine(
                sprintf(" * @paramAddendum %s", $this->tags["param"]["value"])
            );
        }

        if (isset($this->tags["type"])) {
            $this->insertLine(
                sprintf(" * @type %s", $this->tags["type"]["value"])
            );
        }

        if (isset($this->tags["return"])) {
            $this->insertLine(
                sprintf(" * @return %s", $this->tags["return"]["value"])
            );
        }
    }
    ...
    private function findOldStyleTags(): void
    {
        ...
        foreach ($this->source as $key => $line) {
            $n = preg_match(
                '/^\s*--\s*(?<tag>type|return|param)\s*:\s*(?<value>.*)$/',
                $line,
                $matches
            );
            if ($n === 1) {
                $this->tags[$matches["tag"]] = [
                    "key" => $key,
    ...
    private function insertLine(string $line): void
    {
        $n = preg_match(
            '/^\s*\*\s*$/',
            $this->source[$this->docBlockEndKey - 1]
        );
        if ($n === 0) {

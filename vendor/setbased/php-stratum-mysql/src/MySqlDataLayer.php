...
class MySqlDataLayer
{
        ...
        do {
            ...
            if ($result) {
                ...
                foreach ($result->fetch_fields() as $key => $column) {
                    ...
                    $columns[$key]["length"] = max(
                        4,
                        $column->max_length,
                        mb_strlen($column->name)
                    );
                }
                ...
                while ($row = $result->fetch_row()) {
                    ...
                    foreach ($row as $i => $value) {
                        $this->executeTableShowTableColumn(
                            $columns[$i],
                            $value
                        );
                        echo "|";
    ...
    public function getMaxAllowedPacket(): int
    {
        if ($this->maxAllowedPacket === null) {
            ...
            $this->chunkSize = (int) min(
                $this->maxAllowedPacket - 8,
                1024 * 1024
            );
        }
     ...
     */
    public function quoteListOfInt(
        $list,
        string $delimiter,
        string $enclosure,
        string $escape
        ...
        if (is_scalar($list)) {
            ...
            throw new LogicException(
                "Unexpected parameter type '%s'. Array or scalar expected.",
                gettype($list)
            );
        }
        ...
        foreach ($list as $number) {
            ...
            if (!is_numeric($number)) {
                throw new LogicException(
                    ...
                    is_scalar($number) ? $number : gettype($number)
                );
            }
    ...
    public function setConnector(MySqlConnector $connector): void
    {
        if ($this->mysqli !== null) {
            throw new \LogicException(
                "Can not set connector of a connected data layer. Disconnect first."
            );
        }
    ...
    {
        return new MySqlDataLayerException(
            $this->mysqli->errno,
            $this->mysqli->error,
            $method
        );
    ...
    protected function queryError(
        ...
        return new MySqlQueryErrorException(
            $this->mysqli->errno,
            $this->mysqli->error,
            $method,
            $query
     ...
     * @throws MySqlDataLayerException
     */
    protected function sendLongData(
        ...
        if ($data !== null) {
            ...
            $p = 0;
            while ($p < $n) {
                $success = @$statement->send_long_data(
                    $paramNr,
                    substr($data, $p, $this->chunkSize)
                );
                if (!$success) {
    ...
    private function executeTableShowHeader(array $columns): void
    {
        ...
        foreach ($columns as $column) {
            $separator .= str_repeat("-", $column["length"] + 2) . "+";
            $spaces =
                $column["length"] + 2 - mb_strlen((string) $column["header"]);
            ...
            $fillerRight =
                $spacesRight > 0 ? str_repeat(" ", $spacesRight) : "";
    ...
    private function executeTableShowTableColumn(array $column, $value): void
    {
        $spaces = str_repeat(
            " ",
            $column["length"] - mb_strlen((string) $value)
        );

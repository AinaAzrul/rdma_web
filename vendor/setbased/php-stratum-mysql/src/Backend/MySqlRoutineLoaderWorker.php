 ...
 */
class MySqlRoutineLoaderWorker extends MySqlWorker implements
    RoutineLoaderWorker
{
    //--------------------------------------------------------------------------------------------------------------------
    /**
    ...
    public function execute(?array $sources = null): int
    {
        ...
        $this->defaultCharacterSet = $this->settings->manString(
            "loader.character_set"
        );
        $this->defaultCollate = $this->settings->manString("loader.collate");
        $this->constantClassName = $this->settings->optString(
            "constants.class"
        );
        $this->nameMangler = $this->settings->optString(
            "wrapper.mangler_class"
        );

        $this->connect();

        $this->stratumMetaData = new StratumMetadataHelper(
            $metadataFilename,
    ...
    private function detectNameConflicts(): void
    {
        ...
        foreach ($sourcesByMethod as $method => $sources) {
            ...
            $this->io->error(
                sprintf(
                    "The following source files would result wrapper methods with equal name '%s'",
                    $method
                )
            );
            $this->io->listing($tmp);
    ...
    {
        $helper = new SourceFinderHelper(
            dirname($this->settings->manString("stratum.config_path"))
        );
    ...
    private function getDuplicates(): array
    {
        ...
        foreach ($this->sources as $source) {
            if (sizeof($lookup[$source["method_name"]]) > 1) {
                ...
                $duplicatesMethods[$source["method_name"]] =
                    $lookup[$source["method_name"]];
            }
        }
    ...
    private function loadStoredRoutines(): void
    {
        ...
        foreach ($this->sources as $filename) {
            ...
            $helper = new RoutineLoaderHelper(
                $this->dl,
                $this->io,
                $sqlModeHelper,
                $filename["path_name"],
    ...
    {
        $key = RowSetHelper::searchInRowSet(
            $this->characterSets,
            "character_set_name",
            $characterSetName
        );
    ...
    private function replacePairsColumnTypes(): void
    {
        ...
        $this->io->text(
            sprintf(
                "Selected %d column types for substitution",
                sizeof($columns)
            )
        );
    }
    ...
    private function replacePairsConstants(): void
    {
        ...
        $this->io->text(
            sprintf(
                "Read %d constants for substitution from <fso>%s</fso>",
                sizeof($constants),

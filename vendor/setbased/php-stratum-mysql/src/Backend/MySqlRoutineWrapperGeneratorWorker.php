 ...
 */
class MySqlRoutineWrapperGeneratorWorker extends MySqlWorker implements
    RoutineWrapperGeneratorWorker
{
    ...
    {
        $this->wrapperClassName = $this->settings->optString(
            "wrapper.wrapper_class"
        );
        if ($this->wrapperClassName !== null) {
            $this->parentClassName = $this->settings->manString(
                "wrapper.parent_class"
            );
            $this->nameMangler = $this->settings->manString(
                "wrapper.mangler_class"
            );
            $this->wrapperFilename = $this->settings->manString(
                "wrapper.wrapper_file"
            );
            $this->metadataFilename = $this->settings->manString(
                "loader.metadata"
            );
            $this->strictTypes = $this->settings->manBool(
                "wrapper.strict_types",
                true
            );
        }
    }
    ...
    {
        $stratumMetadata = new StratumMetadataHelper(
            $this->metadataFilename,
            RoutineLoaderHelper::METADATA_REVISION
        );
    ...
    private function writeClassHeader(): void
    {
        ...
        $parentClassName = substr(
            ...
            strrpos($this->parentClassName, "\\") + 1
        );
        ...
        $this->codeStore->append(
            sprintf("class %s extends %s", $className, $this->parentClassName)
        );
        $this->codeStore->append("{");
    }
     ...
     */
    private function writeRoutineFunction(
        array $routine,
        NameMangler $nameMangler
    ): void {
        $wrapper = Wrapper::createRoutineWrapper(
            $routine,
            $this->codeStore,
            $nameMangler
        );

...
class MySqlConstantWorker extends MySqlWorker implements ConstantWorker
{
    ...
    {
        $this->constantsFilename = $this->settings->optString(
            "constants.columns"
        );
        ...
        foreach ($this->oldColumns as $table) {
            foreach ($table as $column) {
                ...
                if ($column["constant_name"] == "*") {
                    ...
                    $this->oldColumns[$tableName][$columnName][
                        "constant_name"
                    ] = $constantName;
                } else {
            ...
            switch ($step) {
                case 1:
                    ...
                    if (is_array($token) && $token[0] == T_DOC_COMMENT) {
                        if (
                            strpos($token[1], "@setbased.stratum.constants") !==
                            false
                        ) {
                            $line1 = $token[2];
                            $step = 2;
    ...
    private function fillConstants(): void
    {
        foreach ($this->columns as $tableName => $table) {
            foreach ($table as $columnName => $column) {
                if (
                    isset(
                        $this->columns[$tableName][$columnName]["constant_name"]
                    )
                ) {
                    $this->constants[$column["constant_name"]] =
                        $column["length"];
                }
            }
        }
                ...
                if ($line != "\n") {
                    ...
                    if ($n == 0) {
                        throw new RuntimeException(
                            "Illegal format at line %d in file '%s'.",
                            $lineNumber,
    ...
    private function logNumberOfConstants(): void
    {
        ...
        $this->io->text(
            sprintf(
                "Number of constants based on column widths: %d",
                $countWidths
            )
        );
        $this->io->text(
            sprintf("Number of constants based on database IDs : %d", $countIds)
        );
    }
    ...
    private function writeConstantClass(): void
    {
        ...
        $tmp2 = array_splice(
            $sourceLines,
            isset($lineNumbers[2]) ? $lineNumbers[2] - $lineNumbers[1] : 0
        );

...
class DiffTable
{
    ...
    public function print(AuditStyle $io, bool $full): void
    {
        ...
        if ($full || $this->hasDifferences()) {
            ...
            $table = new Table($io);
            $table
                ->setHeaders(["column", "audit table", "config / data table"])
    ...
    private function rowsEnhanceWithColumnTypeInfo(): void
    {
        foreach ($this->rows as &$row) {
            if ($row["type"] === "column") {
                ...
                $row["rowspan"] =
                    $row["data2"] === null && $row["audit2"] === null ? 1 : 2;
            }
        }
    }
    ...
    private function rowsEnhanceWithDiffIndicator(): void
    {
        foreach ($this->rows as &$row) {
            switch ($row["type"]) {
                case "column":
                    $row["diff"] =
                        isset($row["audit"]) !== isset($row["data"]) ||
                        $row["audit"]->getColumnDefinition() !==
                            $row["data"]->getColumnDefinition();
                    break;

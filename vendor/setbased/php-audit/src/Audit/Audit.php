...
class Audit
{
    ...
    public function __construct(TypedConfig $config, AuditStyle $io)
    {
        ...
        $this->additionalAuditColumns = AuditDataLayer::$dl->resolveCanonicalAdditionalAuditColumns(
    ...
    {
        $this->dataSchemaTables = AuditDataLayer::$dl->getTablesNames(
            $this->config->getManString("database.data_schema")
        );
        $this->auditSchemaTables = AuditDataLayer::$dl->getTablesNames(
            $this->config->getManString("database.audit_schema")
        );
    }
    ...
    public function obsoleteTables(): void
    {
        foreach ($this->config->getManArray("tables") as $tableName => $dummy) {
            if (
                RowSetHelper::searchInRowSet(
                    $this->dataSchemaTables,
                    "table_name",
                    $tableName
                ) === null
            ) {
                $this->io->writeln(
                    sprintf(
                        "<info>Removing obsolete table %s from config file</info>",
                        $tableName
                    )
                );
    ...
    public function unknownTables(): void
    {
        foreach ($this->dataSchemaTables as $table) {
            if (
                $this->config->getOptArray("tables." . $table["table_name"]) !==
                null
            ) {
                if (
                    $this->config->getOptBool(
                        "tables." . $table["table_name"] . ".audit"
                    ) === null
                ) {
                    $this->io->writeln(
                        sprintf(
                            "<info>Audit not set for table %s</info>",
                            $table["table_name"]
                        )
                    );
                } else {
                    if (
                        $this->config->getManBool(
                            "tables." . $table["table_name"] . ".audit"
                        )
                    ) {
                        if (
                            $this->config->getOptString(
                                "tables." . $table["table_name"] . ".alias"
                            ) === null
                        ) {
                            $this->config
                                ->getConfig()
                ...
                $this->io->writeln(
                    sprintf(
                        "<info>Found new table %s</info>",
                        $table["table_name"]
                    )
                );
                $config = $this->config->getConfig();
                $config->set("tables." . $table["table_name"], [
                    "audit" => null,
                    "alias" => null,
                    "skip" => null,
                ]);
    ...
    {
        foreach ($this->dataSchemaTables as $table) {
            $audit = $this->config->getOptBool(
                "tables." . $table["table_name"] . ".audit"
            );
            if ($audit === true) {
                $currentTable = new AuditTable(
                    $this->io,
                    $this->config->getManString("database.data_schema"),
                    $this->config->getManString("database.audit_schema"),
                    $table["table_name"],
                ...
                $currentTable->main(
                    $this->config->getManArray("additional_sql")
                );
            } elseif ($audit === false) {
                AuditTable::dropAuditTriggers(
                    $this->io,
                    $this->config->getManString("database.data_schema"),
                    $table["table_name"]
                );

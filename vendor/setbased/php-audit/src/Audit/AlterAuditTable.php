...
class AlterAuditTable
{
    ...
    {
        $dataTable = $this->getTableMetadata(
            $this->config->getManString("database.data_schema"),
            $tableName
        );
        $auditTable = $this->getTableMetadata(
            $this->config->getManString("database.audit_schema"),
            $tableName
        );
    ...
    private function compareTableColumns(
        TableMetadata $dataTable,
        TableMetadata $auditTable
    ): void {
        $diff = TableColumnsMetadata::differentColumnTypes(
     ...
     * @param TableMetadata $auditTable The metadata of the audit table.
     */
    private function compareTableOptions(
        TableMetadata $dataTable,
        TableMetadata $auditTable
    ): void {
        ...
        if (!empty($options)) {
            ...
            foreach ($options as $option) {
                switch ($option) {
                    case "engine":
                        $parts[] =
                            "engine " . $dataTable->getProperty("engine");
                        break;

                    case "character_set_name":
                        $parts[] =
                            "default character set " .
                            $dataTable->getProperty("character_set_name");
                        break;

                    case "table_collation":
                        $parts[] =
                            "default collate " .
                            $dataTable->getProperty("table_collation");
                        break;
    ...
    private function getTableList(): array
    {
        $tables1 = [];
        foreach (
            $this->config->getManArray("tables")
            as $tableName => $config
        ) {
            if ($config["audit"]) {
                $tables1[] = $tableName;
        ...
        $tables = AuditDataLayer::$dl->getTablesNames(
            $this->config->getManString("database.data_schema")
        );
        $tables2 = [];
        foreach ($tables as $table) {
    ...
    private function getTableMetadata(
        ...
        $columns = AuditDataLayer::$dl->getTableColumns(
            $schemaName,
            $tableName
        );

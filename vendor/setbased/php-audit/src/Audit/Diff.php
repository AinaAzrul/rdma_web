...
class Diff
{
    ...
    private function currentAuditTable(string $tableName): void
    {
        ...
        $auditTableColumns = new TableColumnsMetadata(
            $columns,
            "AuditColumnMetadata"
        );
        ...
        $dataTableColumns = TableColumnsMetadata::combine(
        ...
        foreach ($auditTableColumns->getColumns() as $column) {
            if (
                !in_array(
                    $column->getName(),
                    $this->additionalAuditColumns->getColumnNames()
                )
            ) {
                $column->unsetDefault();
            }
        ...
        $dataTableOptions = AuditDataLayer::$dl->getTableOptions(
            $this->config->getManString("database.data_schema"),
            $tableName
        );
        $auditTableOptions = AuditDataLayer::$dl->getTableOptions(
            $this->config->getManString("database.audit_schema"),
            $tableName
        );
    ...
    private function getTableLists(): array
    {
        ...
        return [
            "current" => array_intersect($tables1, $tables2, $tables3),

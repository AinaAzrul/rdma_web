...
class AuditTable
{
    ...
    public function __construct(
        ...
        $this->dataTableColumnsDatabase = new TableColumnsMetadata(
            $this->getColumnsFromInformationSchema()
        );
     ...
     */
    public static function dropAuditTriggers(
        AuditStyle $io,
        string $schemaName,
        string $tableName
    ): void {
        $triggers = AuditDataLayer::$dl->getTableTriggers(
            $schemaName,
            $tableName
        );
        foreach ($triggers as $trigger) {
            if (
                ...
                $io->logVerbose(
                    "Dropping trigger <dbo>%s</dbo> on <dbo>%s.%s</dbo>",
                    $trigger["trigger_name"],
                    $schemaName,
                    $tableName
                );

                AuditDataLayer::$dl->dropTrigger(
                    $schemaName,
                    $trigger["trigger_name"]
                );
            }
    ...
    {
        $this->io->logInfo(
            "Creating audit table <dbo>%s.%s<dbo>",
        ...
        $columns = TableColumnsMetadata::combine(
            $this->additionalAuditColumns,
            $dataTableColumnsDatabase
        );
        AuditDataLayer::$dl->createAuditTable(
            $this->dataSchemaName,
            $this->auditSchemaName,
            $this->tableName,
            $columns
    ...
    private function addNewColumns(TableColumnsMetadata $columns): void
    {
        ...
        AuditDataLayer::$dl->addNewColumns(
            $this->auditSchemaName,
            $this->tableName,
            $alterColumns
        );
     ...
     */
    private function alterNewColumns(
        ...
        foreach ($newColumns->getColumns() as $newColumn) {
            ...
            $alterNewColumns->appendTableColumn(
                new AlterColumnMetadata($properties)
            );
        }
     ...
     */
    private function createTableTrigger(
        string $action,
        ?string $skipVariable,
        array $additionSql
    ): void {
        ...
        $this->io->logVerbose(
            "Creating trigger <dbo>%s.%s</dbo> on table <dbo>%s.%s</dbo>",
            $this->dataSchemaName,
            $triggerName,
            $this->dataSchemaName,
            $this->tableName
        ...
        AuditDataLayer::$dl->createAuditTrigger(
            $this->dataSchemaName,
            $this->auditSchemaName,
            $this->tableName,
            $triggerName,
    ...
    {
        return AuditDataLayer::$dl->getTableColumns(
            $this->dataSchemaName,
            $this->tableName
        );
    }
    ...
    {
        $actual = new TableColumnsMetadata(
            AuditDataLayer::$dl->getTableColumns(
                $this->auditSchemaName,
                $this->tableName
            )
        );
        $target = TableColumnsMetadata::combine(
            $this->additionalAuditColumns,
            $this->dataTableColumnsDatabase
        );
        $target->enhanceAfter();
        ...
        $altered = TableColumnsMetadata::differentColumnTypes(
            $actual,
            $target,
            ["is_nullable"]
        );
     ...
     */
    private function logColumnInfo(
        TableColumnsMetadata $newColumns,
        TableColumnsMetadata $obsoleteColumns,
        TableColumnsMetadata $alteredColumns
    ): void {
        foreach ($newColumns->getColumns() as $column) {
            $this->io->logInfo(
                "New column <dbo>%s.%s</dbo>",
                $this->tableName,
                $column->getName()
            );
        }
        ...
        foreach ($alteredColumns->getColumns() as $column) {
            $this->io->logInfo(
                "Type of <dbo>%s.%s</dbo> has been altered to <dbo>%s</dbo>",
                $this->tableName,
                $column->getName(),

...
class BaseCommand extends Command
{
    ...
    public function readConfigFile(): void
    {
        ...
        if ($credentials !== null) {
            $tmp = new TypedConfig(
                new Config(dirname($this->configFileName) . "/" . $credentials)
            );
    ...
    {
        $connector = new MySqlDefaultConnector(
            $this->config->getManString("database.host"),
    ...
    protected function writeTwoPhases(string $filename, string $data): void
    {
        ...
        if ($write_flag) {
            ...
            $this->io->text(
                sprintf(
                    "Wrote <fso>%s</fso>",
                    OutputFormatter::escape($filename)
                )
            );
        } else {
            $this->io->text(
                sprintf(
                    "File <fso>%s</fso> is up to date",
                    OutputFormatter::escape($filename)
                )
            );
        }

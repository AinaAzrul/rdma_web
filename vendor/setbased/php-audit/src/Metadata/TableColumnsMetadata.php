...
class TableColumnsMetadata
{
    ...
    public function __construct(
        array $columns = [],
        string $type = "ColumnMetadata"
    ) {
        foreach ($columns as $column) {
            $this->columns[$column["column_name"]] = static::columnFactory(
                $type,
                $column
            );
        }
     ...
     * @return TableColumnsMetadata
     */
    public static function differentColumnTypes(
        TableColumnsMetadata $columns1,
        TableColumnsMetadata $columns2,
        array $ignore = []
    ): TableColumnsMetadata {
        ...
        foreach ($columns1->columns as $column_name => $column1) {
            if (isset($columns2->columns[$column_name])) {
                if (
                    !ColumnMetadata::compare(
                        $column1,
                        $columns2->columns[$column_name],
                        $ignore
                    )
                ) {
     ...
     * @return TableColumnsMetadata
     */
    public static function notInOtherSet(
        TableColumnsMetadata $columns1,
        TableColumnsMetadata $columns2
    ): TableColumnsMetadata {
        $diff = new TableColumnsMetadata();
     ...
     */
    private static function columnFactory(
        string $type,
        array $column
    ): ColumnMetadata {
        switch ($type) {

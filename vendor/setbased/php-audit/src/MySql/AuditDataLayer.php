...
class AuditDataLayer extends MySqlDataLayer
{
    ...
    public function addNewColumns(
        ...
        $helper = new AlterAuditTableAddColumns(
            $auditSchemaName,
            $tableName,
            $columns
        );
    ...
    public function createTemporaryTable(
        ...
        $sql->append(
            sprintf("create table `%s`.`%s` (", $schemaName, $tableName)
        );
        foreach ($auditColumns as $column) {
            $sql->append(
                sprintf("%s %s", $column["column_name"], $column["column_type"])
            );
...
and   t1.TABLE_NAME   = %s',
     ...
     * @return array[]
     */
    public function getTableTriggers(
        string $schemaName,
        string $tableName
    ): array {
        $sql = sprintf(
...
,        TRIGGER_NAME',
     ...
     */
    public function resolveCanonicalAdditionalAuditColumns(
        string $auditSchema,
        array $additionalAuditColumns

...
class Importing
{
     ...
     * @return bool
     */
    private static function collision1(
        array $rawImports,
        ...
        foreach ($rawImports as $rawImport) {
            if (
                $rawImport["fully_qualified_name"] !== $fullyQualifiedName &&
                $rawImport["name"] === $name
            ) {
                return true;
            }
     ...
     * @return bool
     */
    private static function collision2(
        array $rawImports,
        string $fullyQualifiedName,
        string $alias
    ): bool {
    ...
    private function prepare0(): array
    {
        ...
        foreach ($rawImports as &$rawImport) {
            if (
                !$this->isGlobalOrCurrentNamespace($rawImport["namespace"]) &&
                self::collision1(
                    $rawImports,
                    $rawImport["fully_qualified_name"]
                )
            ) {
                $i = 1;
                do {
                    ...
                    self::collision2($rawImports, $this->namespace, $alias)
                );
        ...
        foreach ($rawImports as $rawImport) {
            if ($rawImport["import"]) {
                if ($rawImport["alias"] === null) {
                    $this->imports[] = sprintf(
                        "use %s;",
                        ltrim($rawImport["fully_qualified_name"], "\\")
                    );
                } else {
                    $this->imports[] = sprintf(
                        "use %s as %s;",
    ...
    private function prepare2(array $rawImports): void
    {
        usort($rawImports, function (array $a, array $b) {
            $cmp =
                strlen($b["fully_qualified_name"]) <=>
                strlen($a["fully_qualified_name"]);
            if ($cmp == 0) {
                $cmp =
                    $a["fully_qualified_name"] <=> $b["fully_qualified_name"];
            }
        ...
        foreach ($rawImports as $rawImport) {
            if ($rawImport["namespace"] !== "") {
                $this->replace[$rawImport["fully_qualified_name"]] =
                    $rawImport["alias"] ?? $rawImport["name"];
            }
        }
    }
